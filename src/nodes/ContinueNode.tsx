...
function ContinueNode({ data }: NodeProps) {
  const { label = 'Next section', sectionFraction } = (data ?? {}) as ContinueNodeData;
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      >
        <span className="flex items-center justify-center gap-1.5" aria-hidden>
          <svg
            className="w-7 h-7 animate-slide-right"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.5"
      ...
      <Handle type="source" position={Position.Right} className="!w-2.5 !h-2.5 !bg-teal-400 !border-2 !border-slate-900 opacity-0 pointer-events-none" />
    </div>
  );
}
